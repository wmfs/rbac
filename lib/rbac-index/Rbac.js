...
class Rbac {
  constructor (data) {
    ...
    this.unknownRoles = data.permissions.map(p => p.roleId).filter(roleId => !inheritedBy.hasOwnProperty(roleId))
  ...
  checkRoleAuthorization (userId, ownerId, roles, resourceType, resourceName, action) {
    return checkRoleAuthorization(userId, ownerId, roles, resourceType, resourceName, action, this.index)
